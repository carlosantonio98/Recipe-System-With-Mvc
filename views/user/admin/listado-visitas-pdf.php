...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header
		$w = array(20, 37, 30, 30, 30, 60, 30, 30);
...
$pdf->SetHeaderData('../../../../resources/img/logo.jpg', 33, "Listado de visitas", "Fecha de captura: " . date("Y-m-d"));
...
// column titles
$header = array('Id', 'Nombre Usuario', 'Apellido Paterno', 'Apellido Materno', 'Rol', 'Correo', 'Fecha visita', 'Hora visita');

// data loading database
$data = [];
$db = Connection('finder_food');
$stmt = $db->recordSet('SELECT * FROM vw_visitas', array());

while($row = $db->next($stmt)) {
	$data[] = array($row->IdVisita, $row->NombreUsuario, $row->Paterno, $row->Materno, $row->Rol, $row->Correo, $row->FechaVisita, $row->HoraVisita);
}
