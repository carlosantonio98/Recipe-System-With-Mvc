...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header
		$w = array(20, 37, 30, 30, 30, 30, 60, 30);
...
$pdf->SetHeaderData('../../../../resources/img/logo.jpg', 33, "Listado de usuarios", "Fecha de captura: " . date("Y-m-d"));
...
// column titles
$header = array('Id', 'Nombre Usuario', 'Apellido Paterno', 'Apellido Materno', 'Pais', 'Rol', 'Correo', 'Nacimiento');

// data loading database
$data = [];
$db = Connection('finder_food');
$stmt = $db->recordSet('SELECT * FROM vw_usuarios', array());

while($row = $db->next($stmt)) {
	$data[] = array($row->IdUsuario, $row->NombreUsuario, $row->Paterno, $row->Materno, $row->Pais, $row->Rol, $row->Correo, $row->Nacimiento);
}
